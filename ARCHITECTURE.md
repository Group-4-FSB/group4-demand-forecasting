...
| **Training** | Baseline model, LightGBM + time-series CV + hyperparameter search, a chronological held-out test evaluation, a promotion **quality gate** (rejects a candidate worse than current `production`), MLflow logging & registry promotion | [`src/demand_forecast/models/train.py`](src/demand_forecast/models/train.py) |
